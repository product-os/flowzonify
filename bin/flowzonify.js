...
import { migrateFile, initRepo, WORKFLOW_PATH } from '../lib/run.js';
...
const EXIT = { ok: 0, error: 1, blocked: 2 };
...
function runInit(values) {
	...
	const exit = EXIT[result.migration.status] ?? EXIT.ok;
	...
	console.log(result.created ? `created ${WORKFLOW_PATH}` : `${WORKFLOW_PATH} already exists`);
...
function runMigrate(paths, values) {
 ...
 */
function reportMissingLinter(files) {
	if (files.some((file) => file.changed && file.lintChecked === false)) {
		console.log(
...
function report(file) {
	switch (file.status) {
		...
		case 'blocked':
			...
			for (const entry of file.report.filter((item) => item.status === 'blocked')) {
		...
		default:
			...
			for (const entry of file.report.filter((item) => item.status === 'advisory')) {
