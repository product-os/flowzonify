...
import { validate, ValidationError } from '../lib/validate.ts';
...
test('rejects output that dropped a caller job', () => {
	...
	assert.throws(
		() => validate(LEGACY_CALLER, { ...RESULT, source: without }),
		ValidationError,
	);
});
...
test('rejects output that repointed the caller at a different workflow', () => {
	...
	assert.throws(
		() => validate(LEGACY_CALLER, { ...RESULT, source: repointed }),
		{ message: /uses/ },
	);
});
...
test('rejects output that would change again on a second run', () => {
	const reverted = MIGRATED.replace(
		'  push:\n',
		'  pull_request_target:\n    branches: [main]\n  push:\n',
	);
	assert.throws(
		() => validate(LEGACY_CALLER, { ...RESULT, source: reverted }),
		ValidationError,
	);
});
