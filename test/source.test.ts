...
import { applyEdits, pairFor, pairRange } from '../lib/source.ts';

test('applyEdits replaces a single range', () => {
	assert.equal(
		applyEdits('abcdef', [{ start: 2, end: 4, text: 'XY' }]),
		'abXYef',
	);
});
...
test('applyEdits supports pure insertion at a point', () => {
	assert.equal(
		applyEdits('abcdef', [{ start: 3, end: 3, text: '--' }]),
		'abc--def',
	);
});
...
const doc = (src: string) => parseDocument(src);
const pairOf = (document: Document.Parsed, key: string) =>
	pairFor(document.get('on'), key)!;
