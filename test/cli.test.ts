...
import { WORKFLOW_PATH } from '../lib/run.ts';
...
const makeRepo = (files: Record<string, string> = {}) =>
	repo(files, 'flowzonify-cli-');

const CLI = fileURLToPath(new URL('../bin/flowzonify.ts', import.meta.url));
...
test('--json emits a machine-readable report and nothing else', () => {
	...
	assert.ok(
		report.files[0].report.some((entry) => entry.id === 'add-push-trigger'),
	);
});
...
test('accepts explicit paths', () => {
	...
	assert.equal(result.status, 0, result.stderr);
	assert.ok(
		readFileSync(join(dir, 'other/flowzone.yml'), 'utf8').includes('  push:'),
	);
});
...
test('init exits 1 and lists the options for an unknown repo type', () => {
	...
	assert.match(
		...
		'the error should list what is valid',
	);
});
