...
import { illegalFilters } from '../lib/events.js';
import { parseWorkflow } from '../lib/migrate.js';

const on = (body) => parseWorkflow(`on:\n${body}jobs: {}\n`);
...
test('rejects tag filters on pull_request', () => {
	...
	assert.deepEqual(illegalFilters(doc), ['pull_request.tags', 'pull_request.tags-ignore']);
});

test('says nothing about events it has no table for', () => {
	const doc = on('  workflow_dispatch:\n    inputs:\n      why:\n        type: string\n');
	assert.deepEqual(illegalFilters(doc), []);
});
...
test('says nothing when the triggers are a flow sequence', () => {
	assert.deepEqual(illegalFilters(parseWorkflow('on: [push, pull_request]\njobs: {}\n')), []);
});
