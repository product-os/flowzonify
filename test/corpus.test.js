...
import { migrate, WorkflowParseError, parseWorkflow } from '../lib/migrate.js';
import { callerJobs } from '../lib/context.js';
import { MIGRATIONS } from '../lib/migrations/index.js';
import { validate } from '../lib/validate.js';
import { corpusFixtures } from './helpers.js';

const cases = corpusFixtures();

test('the corpus is not empty', () => {
	assert.ok(cases.length > 0, 'no corpus fixtures found');
});
...
for (const { name, input, expectedPath, context } of cases) {
	...
	if (!existsSync(expectedPath)) {
		test(`${name}: is rejected as unparseable rather than silently mangled`, () => {
			assert.throws(() => migrate(input, undefined, context), WorkflowParseError);
		});
		continue;
	}
	...
	test(`${name}: reformats nothing, it only adds and removes whole lines`, () => {
		const survivable = new Set([...input.split('\n'), ...insertedLines(input, context)]);
...
function insertedLines(input, context) {
...
test('the corpus covers the structural variants that broke naive transforms', () => {
	const all = cases.map((entry) => entry.input);
	const covers = (predicate) => all.some(predicate);

	assert.ok(covers((src) => /branches:\s*\n\s+- /.test(src)), 'a block-sequence branches list');
	assert.ok(covers((src) => /branches: \[/.test(src)), 'a flow-sequence branches list');
	assert.ok(covers((src) => src.includes('pull_request_target')), 'a caller still on pull_request_target');
	assert.ok(covers((src) => !src.includes('pull_request_target')), 'a caller already off pull_request_target');
	assert.ok(covers((src) => src.includes('restrict_custom_actions')), 'a caller setting the deprecated input');
	assert.ok(covers((src) => /^permissions:/m.test(src)), 'a caller pinning a permissions block');
	assert.ok(covers((src) => /^\s{4}\w+: none$/m.test(src)), 'a permissions block indented four spaces');
	assert.ok(covers((src) => /^    with:/m.test(src)), 'a caller passing inputs');
	assert.ok(covers((src) => /^  ci:$/m.test(src)), 'a caller job that is not named flowzone');
	assert.ok(
		covers((src) => /if: \|\n\s+\(\n/.test(src)) && covers((src) => /if: \|\n\s+\(github/.test(src)),
