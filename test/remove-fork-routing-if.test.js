...
import unit from '../lib/migrations/remove-fork-routing-if.js';
import { parseWorkflow } from '../lib/migrate.js';
import { runUnit, caller, LEGACY_CALLER, REFLOWED_ROUTING_IF } from './helpers.js';
...
test('removes the block-scalar routing condition and the comment above it', () => {
	...
	assert.ok(result.source.includes('flowzone.yml@master\n    secrets: inherit\n'));
});

test('removes the reflowed one-clause-per-line variant of the same condition', () => {
	const result = runUnit(unit, caller(REFLOWED_ROUTING_IF));
	assert.equal(result.status, 'applied');
	assert.ok(!result.source.includes('if:'));
});
...
`;
	assert.equal(runUnit(unit, src).status, 'applied');
});
...
test('removes the compact equality spelling of the same routing condition', () => {
	const result = runUnit(unit, caller(COMPACT_ROUTING));
...
test('removes the explanatory comment block that comes with the compact spelling', () => {
	...
	const result = runUnit(unit, documented);
...
test('the compact spelling is idempotent once removed', () => {
	const migrated = runUnit(unit, caller(COMPACT_ROUTING)).source;
	assert.equal(runUnit(unit, migrated).status, 'skip');
});
