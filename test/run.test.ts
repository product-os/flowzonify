...
test('reports blocked and writes nothing when a unit needs a human', () => {
	...
	assert.ok(
		result.report.some((entry) => entry.status === 'blocked' && entry.note),
	);
});
...
test('honours a restricted set of migrations', () => {
	...
	assert.ok(written.includes('  push:'));
	assert.ok(
		written.includes('pull_request_target'),
		'other migrations should not have run',
	);
});

test('rejects an empty migration list rather than guessing what it meant', () => {
	const dir = repo({ [WORKFLOW_PATH]: LEGACY_CALLER });
	assert.throws(
		() => migrateFile(join(dir, WORKFLOW_PATH), { only: [] }),
		/no migration ids/,
	);

	// Refusing has to leave the file alone: running everything would change a
	// caller nobody asked to touch, running nothing would call it up to date.
	assert.equal(readFileSync(join(dir, WORKFLOW_PATH), 'utf8'), LEGACY_CALLER);
});

test('rejects an unknown migration id rather than silently doing nothing', () => {
	const dir = repo({ [WORKFLOW_PATH]: LEGACY_CALLER });
	assert.throws(
		() => migrateFile(join(dir, WORKFLOW_PATH), { only: ['nope'] }),
		/nope/,
	);
});
...
test('init creates a caller workflow that needs no migration', () => {
	...
	writeFileSync(join(dir, WORKFLOW_PATH), created, { encoding: 'utf8' });
...
test('init does not overwrite an existing workflow with the template', () => {
	...
	assert.ok(
		readFileSync(join(dir, WORKFLOW_PATH), 'utf8').includes(
			'docker_images: example-org/example-app',
		),
...
test('a repository with a package.json does not, since versionist assumes node', () => {
	assert.equal(
		needsRepoType(repo({ 'package.json': '{"name":"x"}\n' })),
		false,
	);
});
...
test('an explicit type overrides inference, even for an npm package', () => {
	...
	assert.equal(
		...
		true,
		'an explicit --type must not be silently dropped',
	);
	assert.equal(
		readFileSync(join(dir, 'repo.yml'), 'utf8'),
		'type: python-poetry\n',
	);
});
...
test('init refuses a repository whose type it cannot tell, and touches nothing', () => {
	...
	assert.equal(
		...
		false,
		'no half-initialised repository',
	);
});
...
test('every offered repo type is one balena-versionist recognises', () => {
	for (const type of REPO_TYPES) {
		assert.match(
			type,
			/^[a-z0-9-]+$/i,
			`${type} should be a bare repo.yml type`,
		);
	}
...
test('the template init writes is already a complete caller config', () => {
	const result = migrate(CALLER_TEMPLATE, MIGRATIONS, {
		isNpmPackage: false,
		customActions: [],
	});
	assert.equal(
		result.changed,
		false,
		'init must not write a workflow that is already out of date',
	);
});
...
test('reports whether the schema check actually ran', () => {
	const dir = repo({ [WORKFLOW_PATH]: LEGACY_CALLER });
	const { lintChecked } = migrateFile(join(dir, WORKFLOW_PATH));

	// Which of the two depends on whether actionlint is on this machine. Either way
	// it must not report `skipped`, because the check was asked for.
	assert.ok(
		lintChecked === 'checked' || lintChecked === 'unavailable',
		`unexpected lint state: ${lintChecked}`,
	);
});

test('says the lint was skipped when none was asked for', () => {
	const dir = repo({ [WORKFLOW_PATH]: LEGACY_CALLER });
	assert.equal(
		migrateFile(join(dir, WORKFLOW_PATH), { lint: false }).lintChecked,
		'skipped',
	);
});

test('context comes from the repository the workflow is in', () => {
	// The npm permissions only get added when the repository is a package, so they
	// are the visible proof of which directory the context was read from.
	const dir = repo({
		'package.json': '{"name":"x"}\n',
		[WORKFLOW_PATH]: LEGACY_CALLER,
	});
...
test('context does not come from the directory the tool was invoked in', () => {
	// A workflow outside `.github/workflows/` gets its own directory, which has no
	// package.json. flowzonify's own repository does have one, so leaking the working
	// directory in here would add the npm permissions to a workflow that must not
	// have them.
	const dir = repo({ 'flowzone.yml': LEGACY_CALLER });
	migrateFile(join(dir, 'flowzone.yml'));

	const written = readFileSync(join(dir, 'flowzone.yml'), 'utf8');
	assert.ok(!written.includes('permissions:'), written);
