...
import unit from '../lib/migrations/add-npm-oidc-permissions.js';
import { parseWorkflow } from '../lib/migrate.js';
import { runUnit, caller, LEGACY_CALLER } from './helpers.js';

const run = (src, context = { isNpmPackage: true }) => runUnit(unit, src, context);

const permissionsOf = (src) => parseDocument(src).get('permissions').toJSON();
...
test('puts the permissions block above the jobs it applies to', () => {
	const result = run(LEGACY_CALLER);
	assert.ok(result.source.indexOf('permissions:') < result.source.indexOf('jobs:'));
});
...
test('skips a workflow that already requests id-token: write', () => {
	const already = LEGACY_CALLER.replace('jobs:', 'permissions:\n  id-token: write\n\njobs:');
...
test('adds the permission to an existing block instead of duplicating the key', () => {
	const pinned = LEGACY_CALLER.replace(
		'jobs:',
		'permissions:\n  contents: read\n  packages: write\n\njobs:',
	);
	...
	assert.equal(result.status, 'applied');
	assert.doesNotThrow(() => parseWorkflow(result.source), 'must not produce a duplicate mapping key');
	assert.deepEqual(permissionsOf(result.source), {
		contents: 'read',
		packages: 'write',
		'id-token': 'write',
...
test('keeps the comments in an existing permissions block', () => {
	...
	const result = run(pinned);
	assert.ok(result.source.includes('  # only what flowzone needs\n  contents: read\n'));
});
...
const pinned = (idToken) =>
	LEGACY_CALLER.replace('jobs:', `permissions:\n  contents: read\n  id-token: ${idToken}\n\njobs:`);
...
test('upgrades an id-token that is not write, since only write can publish', () => {
	for (const granted of ['none', 'read']) {
		...
		assert.equal(result.status, 'applied', `expected id-token: ${granted} to be upgraded`);
		assert.equal(permissionsOf(result.source)['id-token'], 'write');
		assert.equal(permissionsOf(result.source).contents, 'read', 'other permissions untouched');
	}
});
...
test('upgrading replaces a comment that described the value being replaced', () => {
	...
	assert.ok(!result.source.includes('locked down by default'), 'the comment now contradicts the value');
	assert.ok(result.source.includes('id-token: write  # https://docs.npmjs.com/trusted-publishers\n'));
});
...
test('upgrades id-token inside a flow-style permissions block, splicing the value alone', () => {
	const flow = LEGACY_CALLER.replace('jobs:', 'permissions: { contents: read, id-token: none }\n\njobs:');
	const result = run(flow);

	assert.equal(result.status, 'applied');
	assert.deepEqual(permissionsOf(result.source), { contents: 'read', 'id-token': 'write' });
});

test('skips a flow-style permissions block that already grants write', () => {
	const flow = LEGACY_CALLER.replace('jobs:', 'permissions: { id-token: write }\n\njobs:');
	assert.equal(run(flow).status, 'skip');
});

test('blocks a flow-style permissions block it cannot extend in place', () => {
	const flow = LEGACY_CALLER.replace('jobs:', 'permissions: { contents: read }\n\njobs:');
	const result = run(flow);
...
const jobGrant = (src) => parseDocument(src).getIn(['jobs', 'flowzone', 'permissions', 'id-token']);
...
test('grants id-token in a job-level permissions block, which the top level cannot reach', () => {
	...
	assert.ok(!/^permissions:/m.test(result.source), 'no workflow-level block the job would ignore');
});
...
test('skips a job-level block that already grants write', () => {
	assert.equal(run(caller('    permissions:\n      id-token: write\n')).status, 'skip');
});
