...
const corrupting = {
	...
	apply: (src) => ({ status: 'applied', edits: [{ start: 0, end: 0, text: 'a: b\n  c: d\n' }] }),
};

test('a unit that throws fails only this workflow', () => {
	const result = migrateSource(LEGACY_CALLER, { units: [exploding], lint: false });
...
test('a unit that produces unparseable source fails only this workflow', () => {
	const result = migrateSource(LEGACY_CALLER, { units: [corrupting], lint: false });
	assert.equal(result.outcome, 'failed');
});
