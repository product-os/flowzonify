...
import unit from '../lib/migrations/migrate-custom-runs-on.ts';
...
const matrixOf = (src: string, key: string) =>
	JSON.parse(
		String(parseDocument(src).getIn(['jobs', 'flowzone', 'with', key])),
	) as unknown;

test('moves the runners into the test matrix when the repo has a custom test action', () => {
	const result = applyUnit(unit, withInput(), { customActions: ['test'] });

	assert.equal(result.status, 'applied');
	assert.deepEqual(matrixOf(result.source, 'custom_test_matrix'), {
		os: JSON.parse(RUNNERS),
	});
	assert.ok(
		!result.source.includes('custom_runs_on'),
		'the deprecated input is gone',
	);
});

test('moves the runners into the publish matrix when that is the custom action', () => {
	const result = applyUnit(unit, withInput('["windows-2022"]'), {
		customActions: ['publish'],
	});

	assert.deepEqual(matrixOf(result.source, 'custom_publish_matrix'), {
		os: ['windows-2022'],
	});
	assert.ok(
		!result.source.includes('custom_test_matrix'),
		'no matrix for an action the repo lacks',
	);
});

test('writes a matrix for every matrix-capable custom action the repo has', () => {
	const result = applyUnit(unit, withInput(), {
...
`);
	const result = applyUnit(unit, src, { customActions: ['test'] });
...
test('drops the input as dead configuration when the repo has no custom actions', () => {
	const result = applyUnit(unit, withInput(), { customActions: [] });

	assert.equal(result.status, 'applied');
	assert.ok(!result.source.includes('custom_runs_on'));
	assert.ok(
		!result.source.includes('_matrix'),
		'no matrix for jobs that do not exist',
	);
	assert.ok(
		!result.source.includes('with:'),
		'an empty `with:` fails actionlint, so it goes too',
	);
});
...
test('keeps the with: block when the dead input has siblings', () => {
	const src = caller(`    with:
      ...
      docker_images: example-org/example-app
`);
	const result = applyUnit(unit, src, { customActions: [] });
...
test('skips a caller that does not use the deprecated input', () => {
	assert.equal(
		applyUnit(unit, LEGACY_CALLER, { customActions: ['test'] }).status,
		'skip',
	);
});
...
test('is idempotent', () => {
	const once = applyUnit(unit, withInput(), { customActions: ['test'] });
	assert.equal(
		applyUnit(unit, once.source, { customActions: ['test'] }).status,
		'skip',
	);
});
...
`;
	assert.equal(
		applyUnit(unit, src, { customActions: ['test'] }).status,
		'skip',
	);
});
...
test('blocks a flow-style with: it cannot splice the input out of', () => {
	const src = caller(
		`    with: { custom_runs_on: '["x"]', docker_images: img }\n`,
	);
	const result = applyUnit(unit, src, { customActions: ['test'] });
