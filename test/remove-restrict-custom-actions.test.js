...
import unit from '../lib/migrations/remove-restrict-custom-actions.js';
import { runUnit, caller } from './helpers.js';
...
test('removes the comment that documents the input', () => {
	const src = caller(`    with:
      ...
      docker_images: example-org/example-app
`);
	assert.ok(!runUnit(unit, src).source.includes('forks may not run our custom actions'));
});

test('blocks a flow-style with: it cannot splice the input out of', () => {
	const src = caller('    with: { restrict_custom_actions: false, docker_images: img }\n');
	const result = runUnit(unit, src);
...
test('skips a caller with no inputs at all', () => {
	assert.equal(runUnit(unit, caller()).status, 'skip');
});

test('skips a workflow that has already been migrated', () => {
	const src = caller('    with:\n      restrict_custom_actions: false\n      docker_images: x\n');
	const migrated = runUnit(unit, src).source;
...
`;
	...
	const jobs = parseDocument(result.source).get('jobs');
