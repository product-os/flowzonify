...
import { pairFor, pairRange } from '../source.js';
 ...
 * gets a human.
 */
const CANONICAL_ROUTING = new Set([
...
export default {
	...
	apply(src, doc) {
		...
		for (const { name, node } of callerJobs(doc)) {
			...
			if (!CANONICAL_ROUTING.has(normalised)) {
	...
	verify(doc) {
		for (const { name, node } of callerJobs(doc)) {
			...
			if (CANONICAL_ROUTING.has(condition.replace(/\s+/g, '')) || condition.includes('pull_request_target')) {
