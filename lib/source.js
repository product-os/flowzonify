...
export function lineStartAt(src, index) {
	return src.lastIndexOf('\n', index - 1) + 1;
}

export function lineEndAt(src, index) {
 ...
 */
export function isBlockMap(node) {
 ...
 */
export function pairFor(map, key) {
	return isMap(map) ? map.items.find((pair) => String(pair.key) === key) : undefined;
}
 ...
 */
export function pairRange(src, pair) {
	...
	if (/\S/.test(src.slice(lineStartAt(src, keyStart), keyStart))) {
		throw new Error('pairRange: the pair does not start its own line; check isBlockMap before splicing');
	}
